...
- Press `F1`, type `/help`, or enter `?` in the prompt to open shell command discovery; textual shell commands now include `/abort`, `/compact`, `/close`, `/queue`, and `/delegate ...` alongside the existing keyboard shortcuts (`CodeAlta`)
