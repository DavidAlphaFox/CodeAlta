...
- Press `Ctrl+G Ctrl+U` or hover/click the footer usage indicator to open the context-usage popup, and `Ctrl+T` or the thread info icon to open the thread report; closing either popup returns focus to the prompt editor so keyboard flow stays intact (`CodeAlta`)
