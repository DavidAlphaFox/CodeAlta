...
- Configurable local raw-API agent runtimes for OpenAI-compatible providers, Anthropic, and Google GenAI, with unified local session journals persisted under `~/.alta/sessions/yyyy/mm/dd/<session-id>.jsonl`, model/provider switches recorded in the journal history, models.dev-backed model metadata enrichment plus per-model override support, and background catalog refreshes used to normalize context-window reporting across providers (`CodeAlta`, `CodeAlta.Agent.*`, `CodeAlta.Catalog`)
