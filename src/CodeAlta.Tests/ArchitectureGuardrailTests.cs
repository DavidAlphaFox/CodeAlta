...
public sealed class ArchitectureGuardrailTests
{
    ...
    public void FrontendCoordinators_DoNotAddUntrackedFireAndForgetTasks()
    {
        ...
        var allowedLegacySites = new HashSet<string>(StringComparer.Ordinal)
        {
            ...
            "App/ShellThreadStateCoordinator.cs:405:_ = _historyLoader.EnsureThreadHistoryLoadedAsync(thread, CancellationToken.None);",
            "App/ShellThreadStateCoordinator.cs:402:_ = _historyLoader.EnsureThreadHistoryLoadedAsync(thread, CancellationToken.None);",
            "App/ShellThreadStateCoordinator.cs:433:_ = _historyLoader.EnsureThreadHistoryLoadedAsync(nextThread, CancellationToken.None);",
            "App/ShellThreadStateCoordinator.cs:425:_ = _historyLoader.EnsureThreadHistoryLoadedAsync(nextThread, CancellationToken.None);",
            "App/ShellThreadStateCoordinator.cs:516:_ = PersistViewStateAsync();",
            ...
            "App/ThreadRuntimeEventCoordinator.cs:258:Task.Run(async () =>",
            "App/ThreadRuntimeEventCoordinator.cs:461:_ = InvalidateProjectFileSearchAsync(thread.WorkingDirectory);",
            "App/ThreadRuntimeEventCoordinator.cs:487:_ = InvalidateProjectFileSearchAsync(thread.WorkingDirectory);",
            ...
            "Presentation/Tabs/ThreadTabStripCoordinator.cs:495:_ = CloseTabFromViewAsync(currentThreadId, ShellTabCloseReason.UserDetached);",
            "Presentation/Tabs/ThreadTabStripCoordinator.cs:701:_ = CloseTabFromViewAsync(CodeAltaApp.DraftTabId, ShellTabCloseReason.UserDetached);",
            "Presentation/Tabs/ThreadTabStripCoordinator.cs:736:_ = CloseTabFromViewAsync(currentTabId, ShellTabCloseReason.FileEditorClosed);",
            "Presentation/Tabs/ThreadTabStripCoordinator.cs:777:_ = CloseTabFromViewAsync(currentTabId, ShellTabCloseReason.UserDetached);",
    ...
    public void ThreadDraftPersistence_UsesMachineSavedPromptsAndDeleteHooks()
    {
        ...
        Assert.IsTrue(compositionSource.Contains("new ThreadPromptDraftService(frontend.LoadPromptDraft, frontend.DeletePromptDraft)", StringComparison.Ordinal));
        Assert.IsFalse(File.Exists(Path.Combine(GetCodeAltaSourceRoot(), "App", "IThreadStateFrontendPort.cs")));
        ...
        Assert.IsTrue(threadStateSource.Contains("_promptDrafts.DeletePromptDraft(threadId);", StringComparison.Ordinal));
    }
