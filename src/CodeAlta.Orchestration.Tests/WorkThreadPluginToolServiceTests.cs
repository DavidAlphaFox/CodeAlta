...
[TestClass]
public sealed class WorkThreadPluginToolServiceTests
{
    [TestMethod]
    ...
    public void MergeTools_ReturnsSourceOptionsWhenNoPluginToolsApply()
    {
        var bridge = new PluginOrchestrationBridge(
            ...
            static () => []);
        var service = new WorkThreadPluginToolService(bridge);
    ...
    private static WorkThreadPluginToolService CreateService(AgentToolDefinition tool)
    {
        ...
        return new WorkThreadPluginToolService(new PluginOrchestrationBridge(
