...
/// <summary>
/// Resolves project-file references in work-thread prompts and records reference usage.
/// </summary>
public interface IWorkThreadPromptReferenceService
{
    /// <summary>
    ...
    ValueTask<WorkThreadPromptReferenceResult> ResolveAsync(
        string prompt,
        string? projectRoot,
...
/// <summary>
/// Project-file-backed implementation of <see cref="IWorkThreadPromptReferenceService"/>.
/// </summary>
public sealed class WorkThreadPromptReferenceService : IWorkThreadPromptReferenceService
{
    private readonly IProjectFileSearchService _searchService;

    /// <summary>
    /// Initializes a new instance of the <see cref="WorkThreadPromptReferenceService"/> class.
    /// </summary>
    ...
    public WorkThreadPromptReferenceService(IProjectFileSearchService searchService)
    {
    ...
    /// <inheritdoc />
    public async ValueTask<WorkThreadPromptReferenceResult> ResolveAsync(
        string prompt,
        string? projectRoot,
        CancellationToken cancellationToken = default)
    {
        ...
        if (tokens.Count == 0)
        {
            return new WorkThreadPromptReferenceResult(prompt, AgentInput.Text(prompt), []);
        }
        ...
        {
            return new WorkThreadPromptReferenceResult(normalizedPrompt, AgentInput.Text(normalizedPrompt), []);
        }
        ...
        return new WorkThreadPromptReferenceResult(normalizedPrompt, new AgentInput(items), resolvedReferences);
    }
...
public sealed record WorkThreadPromptReferenceResult(
